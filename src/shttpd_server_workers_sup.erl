...
%%%-------------------------------------------------------------------
-module(shttpd_server_workers_sup).
...
%%--------------------------------------------------------------------
init(Server) -> {ok, {{one_for_one, 10000, 1}, workers(Server)}}.
...
workers(Server = #{workers := Workers}) ->
    [conn(Id, Server) || Id <- lists:seq(1, Workers)];
workers(_) ->
    [].

conn(Id, Server) ->
    {Id, {shttpd_server_worker, start_link, [Server]},
     permanent, 5000, worker, [shttpd_server_worker]}.
